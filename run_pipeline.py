...
OUTPUT_DIR = Path.home() / "cookcounty_pipeline_output"
OUTPUT_DIR.mkdir(exist_ok=True)

OUTPUT_EXCEL = OUTPUT_DIR / "final_pipeline_results.xlsx"
...
for _, _, _, json_file in PHASES:
    path = OUTPUT_DIR / json_file
    if not path.exists():
        with open(path, "w", encoding="utf-8") as f:
            json.dump([], f)
        print(f"📝 Created empty JSON: {path.name}")
...
def run_phase(name, script):
    ...
    if result.returncode != 0:
        ...
        print(f"⚠ {name} stopped (likely no new data). Continuing...")
        return False
...
def main():
    for name, script, csv_file, _ in PHASES:
        ...
        # Consolidate CSV into centralized Excel
